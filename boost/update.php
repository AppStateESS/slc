...
<pre>';
        ...
        case version_compare($current_version, '3.0.4', '<'):
             ...
             */
        case version_compare($current_version, '3.0.5', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/slc/boost/updates/update_3_0_5.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
        case version_compare($current_version, '3.0.6', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/slc/boost/updates/update_3_0_6.sql');
            if (PEAR::isError($result)) {
                return $result;
            }            
    }
