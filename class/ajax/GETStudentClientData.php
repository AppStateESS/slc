...
class GETStudentClientData extends AJAX {
    ...
    public function execute() {
        ...
        $client = ClientFactory::getClientByEncryBanner($encryptedBanner, $student->getFirstName(), $student->getLastName(), $student->getFirstName() . ' ' . $student->getLastName());

        // Determines if the client is new
        if ($client == null) {
            $client = new \slc\Client($encryptedBanner, $student->getClassification(), $student->getMajor(), $student->getLivingLocation());
            ...
            $client->setRace($student->getRace());
